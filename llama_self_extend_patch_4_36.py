...
def apply_grouped_rotary_pos_emb(
    q, k, cos, sin, position_ids, g_size_1=8, g_size_2=1024
):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
...
def self_extend_forward(
    ...
    **kwargs,
    ...
    key_position_ids = (
        ...
        .to(query_position_ids.device)
        .view(bsz, kv_seq_len)
    )

    neighbor_query_states, _ = apply_rotary_pos_emb(
        query_states, None, cos, sin, query_position_ids
    )
    _, neighbor_key_states = apply_rotary_pos_emb(
        None, key_states, cos, sin, key_position_ids
    )
    _re_group_size_2 = (
